  ...
  apply(compiler) {
    compiler.hooks.emit.tap("ImsWebpackPlugin", compilation => {
      ...
      hooks.htmlWebpackPluginAfterEmit.tapAsync(
        "ImsWebpackPlugin",
        async (item, callback) => {
          ims_webpack_dll_1.imsConfig
            .get("dll")
            .map(str => html.addScript(str));

          for (let assetName in compilation.assets) {
            if (assetName.lastIndexOf(".js") > -1) {
              await this.addScript(compilation, assetName);
            }
            if (assetName.lastIndexOf(".css") > -1) {
              await this.addStyle(compilation, assetName);
            }
          }
